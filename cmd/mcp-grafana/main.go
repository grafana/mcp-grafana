...
func newServer(serverName string, dt disabledTools, obs *observability.Observability, instructionsAppend string) *mcp.Server {
	...
	// OrgID middleware is registered per-transport, AFTER GrafanaContextMiddleware
	// (for HTTP transports), so the org override runs on the populated config.
	// For stdio, it's registered here since there's no per-call context middleware.
...
// surfaces when the stdio transport shuts down. The SDK wraps an unexported
// error type, so we match on the message text.
func isBenignStdioClose(err error) bool {
	return err != nil && strings.Contains(err.Error(), "server is closing")
}

// runHTTPServerDirect starts an *http.Server and blocks until ctx is cancelled
// or the server returns an error. On context cancellation it performs a graceful
// shutdown with a 5-second deadline.
func runHTTPServerDirect(ctx context.Context, srv *http.Server, transportName string) error {
...
// runHTTPServerTLS is like runHTTPServerDirect but uses TLS.
func runHTTPServerTLS(ctx context.Context, srv *http.Server, certFile, keyFile, transportName string) error {
	serverErr := make(chan error, 1)
	go func() {
...
func run(transport, addr, basePath, endpointPath string, logLevel slog.Level, dt disabledTools, gc mcpgrafana.GrafanaConfig, tls tlsConfig, hsc httpSecurityConfig, ca callerAuthConfig, obs observability.Config, healthzAddress, instructionsAppend string) error {
	...
	case "stdio":
		...
		if err != nil && !isBenignStdioClose(err) && err != context.Canceled {
	...
	case "sse":
		...
		return runHTTPServerDirect(ctx, httpSrv, "SSE")
	...
	case "streamable-http":
		...
		if tls.certFile != "" || tls.keyFile != "" {
			return runHTTPServerTLS(ctx, httpSrv, tls.certFile, tls.keyFile, "StreamableHTTP")
		}
		return runHTTPServerDirect(ctx, httpSrv, "StreamableHTTP")
...
func main() {
	...
	if err := run(transport, *addr, *basePath, *endpointPath, level, dt, grafanaConfig, tls, hsc, ca, obs, *healthzAddress, *instructionsAppend); err != nil {
		panic(err)
	}
}
