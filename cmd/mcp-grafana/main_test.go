...
func TestProcessTools_EnableWriteToolsRestoresDashboardUpdate(t *testing.T) {
	names := registerAllCategories(t, disabledTools{write: true, writeToolOverrides: "update_dashboard"})
	assert.True(t, names["update_dashboard"], "update_dashboard should be restored by --enable-write-tools")
	assert.False(t, names["create_datasource"], "dashboard override must not re-enable datasource writes")
	assert.False(t, names["create_folder"], "dashboard override must not re-enable unrelated write tools")
}
