	...
	for _, disableWrite := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, tool := range listAllTools(t, disableWrite) {
				...
				var missing []string
				// ReadOnlyHint is a bool (false is meaningful, default is false)
				// so we can't check for nil; but DestructiveHint and OpenWorldHint
				// are *bool so nil means not set.
