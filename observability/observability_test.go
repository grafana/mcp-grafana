...
// testBuildOperationAttrs wraps buildOperationAttrs with the pre-extraction
// that the middleware normally performs, so tests can keep using the simpler
// (ctx, method, req, result, err) call pattern.
func (o *Observability) testBuildOperationAttrs(ctx context.Context, method string, req mcp.Request, result mcp.Result, err error) []attribute.KeyValue {
	toolName, toolNameOK := toolNameFromRequest(method, req)
	args := toolArgsFromRequest(method, req)
	return o.buildOperationAttrs(ctx, method, toolName, toolNameOK, args, req, result, err)
}
...
func TestBuildOperationAttrs(t *testing.T) {
	...
	t.Run("basic method attrs", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(ctx, "tools/list", nil, nil, nil)
	...
	t.Run("error includes error.type", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(ctx, "tools/call", nil, nil, testErr)
	...
	t.Run("tools/call with wrong-type request does NOT emit gen_ai.tool.name", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(ctx, "tools/call", nil, nil, nil)
		for _, a := range attrs {
	...
	t.Run("non-tools/call method with valid CallToolRequest does NOT emit gen_ai.tool.name", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(ctx, "tools/list", req, nil, nil)
		for _, a := range attrs {
			assert.NotEqual(t, "gen_ai.tool.name", string(a.Key),
	...
	t.Run("tools/call includes phase declared on the result", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(ctx, "tools/call", req, res, nil)
...
func TestToolMetricDimensions(t *testing.T) {
	...
	t.Run("parity with buildOperationAttrs for an allowlisted tool", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(context.Background(), "tools/call", req, res, nil)
...
func TestToolMetricDimensionsPhaseIsBounded(t *testing.T) {
	...
	t.Run("buildOperationAttrs drops an un-allowlisted tool's phase", func(t *testing.T) {
		...
		attrs := obs.testBuildOperationAttrs(context.Background(), "tools/call", req,
...
func TestEnrichSpanWithToolDims(t *testing.T) {
	...
	t.Run("sets operation, resource_type, target, and phase on a recording span", func(t *testing.T) {
		...
		obs.enrichSpanWithToolDims(ctx, "tools/call", args, res)
		span.End()
	...
	t.Run("no-op when span is not recording", func(t *testing.T) {
		...
		assert.NotPanics(t, func() {
			obs.enrichSpanWithToolDims(context.Background(), "tools/call", args, nil)
		})
	})
}
