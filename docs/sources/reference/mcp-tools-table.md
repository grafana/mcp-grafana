...
You can limit which tools the server exposes with `--enabled-tools`, `--disable-<category>`, `--disable-write`, and `--disable-query`. Refer to [Enable and disable tools](../../configure/enable-and-disable-tools/) and [Command-line flags](../../configure/command-line-flags/).
