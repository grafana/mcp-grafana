...
| Field | Description | Example |
| :---- | :---- | :---- |
| `grafana_version` | The version the Grafana instance reported, read from a response the session had already fetched for its own reasons, and truncated to 64 bytes. Empty whenever that version is not already known — nothing is fetched to find it out. | `12.1.0` |
...
- **`grafana_version` and the target fields are empty until the session talks to Grafana.** They are read from configuration and from a cache the session's own Grafana requests populate. Reporting never issues a request of its own, so a version that nothing else has fetched is simply absent: a session that opened and closed without calling a tool reports these empty, and so does one on an instance whose `/api/frontend/settings` is unreachable or forbidden. Empty means "not resolved", never "old" or "self-hosted", and `grafana_version` coverage is not a measure of anything about the instance.
- **`grafana_version` is the instance's own string.** Like `client_version` it has no vocabulary to clamp against — forks and internal builds set it freely — so it travels as reported, bounded only by a 64-byte truncation.
...
| Time limit | Five seconds for a background report; one second for the whole shutdown flush, however many sessions it covers, including the time spent building the events rather than only sending them |
