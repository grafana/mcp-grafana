...
| `org_id_set` | Whether a Grafana organisation was selected for **at least one** request this process served. Whether, not which: the organisation ID is never sent. | `true` |
...
- **`org_id_set` is "at least one", not "every".** The organisation can be selected per request by header, so in a multi-tenant process a single request carrying one sets this `true` for the whole report.
