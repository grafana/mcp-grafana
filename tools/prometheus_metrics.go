...
func metricNamesMeter(ctx context.Context) metric.Meter {
	return mcpgrafana.GrafanaConfigFromContext(ctx).MeterProviderOrDefault().Meter("mcp-grafana")
}

func recordMetricNames(ctx context.Context, backend string, count int) {
	histogram, _ := metricNamesMeter(ctx).Int64Histogram("mcp.metric_names.count",
	...
	histogram.Record(ctx, int64(count), metric.WithAttributes(attribute.String("backend", backend)))
}
...
func (t *prometheusMetricNamesTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	resp.Body = &metricNamesResponseBody{ReadCloser: resp.Body, ctx: req.Context()}
	return resp, nil
}

func recordMetricNamesResponseSize(ctx context.Context, backend string, size int64) {
	histogram, _ := metricNamesMeter(ctx).Int64Histogram("mcp.metric_names.response.size",
	...
	histogram.Record(ctx, size, metric.WithAttributes(attribute.String("backend", backend)))
}
...
func (b *metricNamesResponseBody) Read(p []byte) (int, error) {
	...
	if err == io.EOF && !b.recorded {
		recordMetricNamesResponseSize(b.ctx, "prometheus", b.bytes)
		b.recorded = true
	}
	return n, err
}
