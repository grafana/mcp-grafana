...
func TestResolveSiftInputs(t *testing.T) {
	t.Run("labelSelector takes precedence over labels", func(t *testing.T) {
		inputs, err := resolveSiftInputs(
			`{namespace=~"prod.*"}`,
			map[string]string{"namespace": "production"},
		)
		require.NoError(t, err)
		require.Len(t, inputs, 1)
		assert.Equal(t, labels.MatchRegexp, inputs[0].LabelMatcher.Type)
		assert.Equal(t, "prod.*", inputs[0].LabelMatcher.Value)
	})

	t.Run("falls back to labels map", func(t *testing.T) {
		inputs, err := resolveSiftInputs("", map[string]string{"cluster": "us-east-1"})
		require.NoError(t, err)
		require.Len(t, inputs, 1)
		assert.Equal(t, labels.MatchEqual, inputs[0].LabelMatcher.Type)
		assert.Equal(t, "us-east-1", inputs[0].LabelMatcher.Value)
	})

	t.Run("error when both empty", func(t *testing.T) {
		_, err := resolveSiftInputs("", nil)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "either labelSelector or labels must be provided")
	})
}

func TestLabelsMapToInputs(t *testing.T) {
	inputs := labelsMapToInputs(map[string]string{
		"namespace": "production",
		"cluster":   "us-east-1",
	})
	assert.Len(t, inputs, 2)
	for _, input := range inputs {
		assert.Equal(t, "label", input.Type)
		assert.Equal(t, labels.MatchEqual, input.LabelMatcher.Type)
	}
}
