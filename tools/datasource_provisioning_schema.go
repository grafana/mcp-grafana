...
//go:embed datasource_schemas/*.json
...
// loadDatasourceSchema loads the schema for the given plugin type.
// Returns (nil, nil) when no schema file exists for the type.
// Returns (nil, err) when the file exists but cannot be parsed.
func loadDatasourceSchema(pluginType string) (*datasourceSchema, error) {
	data, err := datasourceSchemaFiles.ReadFile(fmt.Sprintf("datasource_schemas/%s_schema.json", pluginType))
	if err != nil {
