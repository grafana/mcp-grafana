...
func queryLokiLogs(ctx context.Context, args QueryLokiLogsParams) (*QueryLokiLogsResult, error) {
	...
	if mcpgrafana.GrafanaConfigFromContext(ctx).LokiGuardrailMode == mcpgrafana.LokiGuardrailStrict {
		if _, native := backend.(*lokiNativeBackend); native {
			logql, err = prepareStrictLokiQuery(logql)
			if err != nil {
				return nil, err
			}
		}
