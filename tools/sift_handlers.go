...
func siftRead(ctx context.Context, args SiftReadParams) (any, error) {
	if err := args.validate(); err != nil {
		return nil, fmt.Errorf("sift_read: %w", err)
	}

	switch args.Operation {
	case "list_investigations":
		return listSiftInvestigations(ctx, ListSiftInvestigationsParams{Limit: args.Limit})
	case "get_investigation":
		return getSiftInvestigation(ctx, GetSiftInvestigationParams{ID: args.InvestigationID})
	case "get_analysis":
		return getSiftAnalysis(ctx, GetSiftAnalysisParams{
			InvestigationID: args.InvestigationID,
			AnalysisID:      args.AnalysisID,
		})
	default:
		// Unreachable once validate() has passed; kept for defense in depth.
		return nil, fmt.Errorf("sift_read: unknown operation %q", args.Operation)
	}
}
...
func siftWrite(ctx context.Context, args SiftWriteParams) (any, error) {
	if err := args.validate(); err != nil {
		return nil, fmt.Errorf("sift_write: %w", err)
	}

	switch args.Operation {
	case "find_error_pattern_logs":
		return findErrorPatternLogs(ctx, args)
	case "find_slow_requests":
		return findSlowRequests(ctx, args)
	default:
		// Unreachable once validate() has passed; kept for defense in depth.
		return nil, fmt.Errorf("sift_write: unknown operation %q", args.Operation)
	}
}
...
func findErrorPatternLogs(ctx context.Context, args SiftWriteParams) (*analysis, error) {
...
func findSlowRequests(ctx context.Context, args SiftWriteParams) (*analysis, error) {
	logger := mcpgrafana.LoggerFromContext(ctx)
	client, err := siftClientFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("creating Sift client: %w", err)
