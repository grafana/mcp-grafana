...
var RunPanelQuery = mcpgrafana.MustTool(
	"run_panel_query",
	"Execute dashboard panel queries with optional time range and variable overrides. Use when the user wants to retrieve actual data from specific Grafana dashboard panels or run panel queries with custom parameters. Accepts `panel_ids` (required array), `dashboard_uid` (required), `time_range` (optional), and `variables` (optional object for template variable overrides). Fetches the dashboard, extracts queries from specified panels, substitutes Grafana macros, and routes to appropriate datasources (Prometheus, Loki, ClickHouse, or CloudWatch). Returns results keyed by panel ID with partial failures allowed. e.g., panel_ids=[\"panel-1\", \"panel-2\"], time_range=\"now-1h\". Do not use when you need to find available panels first (use get_dashboard_summary instead). Raises an error if the dashboard or panels cannot be found.",
	runPanelQuery,
