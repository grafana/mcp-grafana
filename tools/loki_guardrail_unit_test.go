...
func TestParseLogQLSelectorsSelectivity(t *testing.T) {
	tests := []struct {
		...
		{"starred empty-only", `{cluster=~"(^$)*"}`, 1, false},
		{"plussed empty-only", `{cluster=~"(^)+"}`, 1, false},
		{"optional empty-only", `{cluster=~"(^$)?"}`, 1, false},
		{"repeated empty-only", `{cluster=~"(^$){2,3}"}`, 1, false},
		{"starred empty-only alternation", `{cluster=~"(^|$)*"}`, 1, false},
		{"starred empty group", `{cluster=~"()*"}`, 1, false},
...
func TestLokiGuardrailReasonsPartialTotalOverBudgetStillBlocks(t *testing.T) {
	...
	// First leg alone is already over budget; the second leg's stats call
	// fails. Bytes are additive, so the remaining selectors can only push the
	// total higher — the over-budget verdict must stand rather than fail open.
	statsFn := func(ctx context.Context, query string, s, e time.Time) (*Stats, error) {
		if query == `{namespace="a", app="x"}` {
			return &Stats{Bytes: 500 << 30}, nil
		}
		return nil, errors.New("boom")
	}
	logql := `sum(rate({namespace="a", app="x"}[5m])) / sum(rate({namespace="a", app="y"}[5m]))`
