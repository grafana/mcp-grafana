...
func TestMaskingConfig_JSONSerialization(t *testing.T) {
	t.Run("config with builtin patterns", func(t *testing.T) {
...
func TestValidateMaskingConfig_TooManyPatterns(t *testing.T) {
	t.Run("exceeds max pattern limit", func(t *testing.T) {
...
func TestNewLogMasker_PatternCount(t *testing.T) {
	t.Run("pattern count matches builtin patterns", func(t *testing.T) {
		config := &MaskingConfig{
			BuiltinPatterns: []string{"email", "phone"},
		}
		...
		assert.Equal(t, 2, masker.PatternCount())
	})
}
...
func TestLogMasker_MaskEntries_MultipleBuiltinPatterns(t *testing.T) {
	t.Run("multiple builtin patterns are applied", func(t *testing.T) {
		config := &MaskingConfig{
...
func BenchmarkLogMasker_MaskEntries_AllBuiltinPatterns(b *testing.B) {
	// Setup: Create masker with all 7 builtin patterns
	config := &MaskingConfig{
		BuiltinPatterns: []string{
			"email", "phone", "credit_card", "ip_address",
			"mac_address", "api_key", "jwt_token",
...
func BenchmarkLogMasker_MaskEntries_SLO(b *testing.B) {
	...
	// Verify we have 7 patterns
	if masker.PatternCount() != 7 {
		b.Fatalf("Expected 7 patterns, got %d", masker.PatternCount())
	}
...
func BenchmarkLogMasker_PatternCompilation(b *testing.B) {
	// Setup config with all builtin patterns
	config := &MaskingConfig{
		BuiltinPatterns: []string{
			"email", "phone", "credit_card", "ip_address",
			"mac_address", "api_key", "jwt_token",
...
func TestSLOCompliance(t *testing.T) {
	...
	// Verify we have 7 patterns
	require.Equal(t, 7, masker.PatternCount(), "Expected 7 patterns for SLO test")
	...
	assert.Less(t, averageDuration, sloLimit,
		"SLO violation: masking 100 entries with 7 builtin patterns took %v (limit: %v)",
...
func TestMaskerContext_RoundTrip(t *testing.T) {
	t.Run("WithMasker and MaskerFromContext round trip", func(t *testing.T) {
		...
		assert.Equal(t, masker.PatternCount(), retrieved.PatternCount())
	})
