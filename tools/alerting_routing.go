...
// ManageRoutingReadWriteParams extends the read-only routing schema with
// contact-point creation. Write-only inputs are never advertised in read-only mode.
type ManageRoutingReadWriteParams struct {
	ManageRoutingParams
	Operation             string         `json:"operation" jsonschema:"required,enum=get_notification_policies,enum=get_contact_points,enum=get_contact_point,enum=get_time_intervals,enum=get_time_interval,enum=create_contact_point,description=Routing operation to perform"`
	Name                  *string        `json:"name,omitempty" jsonschema:"description=Contact point name: required for create_contact_point or an exact-match filter for get_contact_points"`
	Type                  string         `json:"type,omitempty" jsonschema:"description=Integration type such as email or webhook. Required for create_contact_point."`
	Settings              map[string]any `json:"settings,omitempty" jsonschema:"description=Integration settings object. Required for create_contact_point. Example for email: {\"addresses\":\"team@example.com\"}."`
	UID                   string         `json:"uid,omitempty" jsonschema:"description=Optional contact point UID for creation. Generated by Grafana if omitted."`
	DisableResolveMessage bool           `json:"disable_resolve_message,omitempty" jsonschema:"description=Suppress resolved notifications for the new contact point. Default false."`
	DisableProvenance     *bool          `json:"disable_provenance,omitempty" jsonschema:"description=Allow editing the new contact point in the Grafana UI. Default true."`
}

func manageRoutingReadWrite(ctx context.Context, args ManageRoutingReadWriteParams) (any, error) {
	if args.Operation == "create_contact_point" {
		return createContactPoint(ctx, args)
	}
	read := args.ManageRoutingParams
	read.Operation = args.Operation
	read.Name = args.Name
	return manageRouting(ctx, read)
}
...
var ManageRoutingReadWrite = mcpgrafana.MustTool(
	"alerting_manage_routing",
	manageRoutingDescription+"\n\nUse create_contact_point to create a Grafana-managed contact point with name, type and settings. Returns its UID, name and type without integration settings. External Alertmanager creation is not supported.",
	manageRoutingReadWrite,
	mcp.WithTitleAnnotation("Manage alerting routing"),
