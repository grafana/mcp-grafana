...
- **Create contact points:** Use `alerting_manage_routing` with `operation: "create_contact_point"` to create a Grafana-managed contact point. For example:
  ...
  ```json
  {
    ...
    "name": "Team email",
    "type": "email",
    "settings": {"addresses": "team@example.com"}
  }
  ```

  `name`, `type`, and the integration-specific `settings` object are required. Optional inputs are `uid` (otherwise generated by Grafana), `disable_resolve_message` (default `false`), and `disable_provenance` (default `true`, allowing edits in the Grafana UI). The result contains only `uid`, `name`, and `type`, without integration settings. Creation uses the Grafana provisioning API and requires `alert.provisioning:write` at organization scope (or equivalent provisioning permissions). External Alertmanager creation is not supported. With `--disable-write`, creation and its parameters are unavailable; existing read operations remain available.
...
| `alerting_manage_routing`         | Alerting                  | Read notification policies, contact points, and time intervals; create Grafana-managed contact points        | `alert.notifications:read` (read); `alert.provisioning:write` (create) | Global scope                             |
...
- `alerting_manage_routing` (create_contact_point operation)
