...
Three tools cover the whole domain: `agento11y_read` for the agent catalog, conversations, and generations (no write half exists for any of these); `agento11y_evals_read` and `agento11y_evals_write` for evaluators, eval rules and guards, saved conversations and collections, experiments, and test suites, split into a read-only tool and a write-capable one with disjoint `operation` enums.

- **List and search conversations (`agento11y_read`):** List recent LLM conversations or search them with a filter expression (model, provider, agent, status, error type, eval results, and more) over a time range. Search results include error counts, rating summaries, evaluation summaries, and trace IDs.
- **Get conversation detail (`agento11y_read`):** Fetch a single conversation with all its generations, including prompts and outputs.
- **Get generation detail and scores (`agento11y_read`):** Fetch a single generation by ID, and its evaluation scores (evaluator, score key, value, passed, explanation).
- **Read the agent catalog (`agento11y_read`):** List the agents that send telemetry, fetch one agent version in full (complete system prompt, every tool with its JSON schema, and the models it ran on), walk an agent's version history, and compare evaluation score aggregates per version. Effective versions are `sha256:` hashes that a tool change never affects; for an agent that reports no version of its own they hash the system prompt, so a prompt edit mints a new version. Catalog and version rows carry a `token_estimate`, which is worth checking before fetching a full prompt.
- **Inspect evaluators and templates (`agento11y_evals_read`, writes in `agento11y_evals_write`):** Read the evaluators a score came from, the templates they were derived from, and the judge providers and models available to LLM-judge evaluators. With write tools enabled, also create, fork, test, and delete evaluators.
- **Inspect eval rules and guards (`agento11y_evals_read`, writes in `agento11y_evals_write`):** Read the asynchronous eval rules that bind evaluators to production traffic, and the guards (hook rules) that run inline and can warn or deny. With write tools enabled, also create, update, preview, and delete them. Writes and the non-persisting `preview_rule` and `test_evaluator` operations need the `grafana-agento11y-app.eval:write` permission, granted by the Agento11y Admin role.
- **Curate saved conversations and collections (`agento11y_evals_read`, writes in `agento11y_evals_write`):** Read the saved conversations (bookmarks that give a conversation a stable ID, name, and tags) and the collections that group them, including each collection's member count and the collections embedded in every saved-conversation row. With write tools enabled, also bookmark a conversation, create and edit collections, and add or remove members. These writes need the same `grafana-agento11y-app.eval:write` permission.
- **Read and edit test suites (`agento11y_evals_read`, writes in `agento11y_evals_write`):** List the versioned test suites that offline experiments run against, read one with its full version history, and page through the test cases of a version. With write tools enabled, also create a suite, rename or retag it, open a draft version, publish it, and write or delete its test cases. A published version is frozen, so an edit means opening a new draft. These writes need `grafana-agento11y-app.eval:write`.
- **Read offline experiments (`agento11y_evals_read`, writes in `agento11y_evals_write`):** List the evaluation runs over a test suite and read one with its headline pass rate, cost, and token totals. Drill down through a per-test-case report to trials, their scores with each judge's explanation, and their artifact metadata. With write tools enabled, also rename or retag an experiment and cancel a running one, which need `grafana-agento11y-app.eval:write`. Experiments are created by SDK runners, not by this tool.
...
| `agento11y_read` | Agent Observability*  | Read the agent catalog, list and search LLM conversations, and fetch generation details and evaluation scores from Grafana Agent Observability | `grafana-agento11y-app.conversations:read` and `grafana-agento11y-app.data:read` | N/A                                                 |
| `agento11y_evals_read` | Agent Observability* | Read evaluators and templates, eval rules and guards, saved conversations and collections, offline experiments, and test suites | `grafana-agento11y-app.data:read`                          | N/A                                                 |
| `agento11y_evals_write` | Agent Observability* | Create, update, delete, preview, or test evaluators, eval rules and guards, saved conversations and collections, experiments, and test suites | `grafana-agento11y-app.eval:write`                          | N/A                                                 |
...
- `agento11y_evals_write` (upsert/delete/fork/test evaluator operations; create/update/delete/preview eval rule and guard operations; save/delete saved conversations and create/update/delete collections and their members; update/cancel experiment operations; create/update test suites, create/publish versions, and upsert/delete test cases)
