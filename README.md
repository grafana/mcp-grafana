...
- `--loki-guardrail-mode`: Loki query cost guardrail for `query_loki_logs` and `query_loki_patterns` - default: `off`. Loki does not enforce `max_query_bytes_read` on log queries without a line filter, so a broad selector over a wide range can scan terabytes; the guardrail requires a selective stream selector, caps the effective time range (including range-vector durations like `[30d]`), and pre-checks Loki's index/stats byte estimate before running the query. `shadow` logs queries that would be blocked but lets them run (it still pays the index/stats round trip); `enforce` rejects known violations but retains fail-open compatibility when parsing or estimation fails; `strict` also rejects incomplete evaluations and refuses to start while bypass-capable tool categories are enabled. On VictoriaLogs, `shadow` and `enforce` apply only to selector-shaped (`{...}`) queries, while `strict` rejects content queries because no cheap byte estimate is available. Env fallback: `GRAFANA_LOKI_GUARDRAIL_MODE`.
...
`strict` is the fail-closed MCP option, but the byte decision is still based on Loki's index/stats estimate. That endpoint can be approximate and can omit data still held by ingesters, so this is not an exact backend-enforced byte ceiling. Configure headroom below the operational limit and keep a short range cap. Strict mode also requires `api`, `rendering`, `sift`, `assistant`, and proxied tools to be disabled when present, so Loki content cannot take a path around the guarded tools. `run_panel_query` remains compatible because its Loki execution delegates to `query_loki_logs`.
