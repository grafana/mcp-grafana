...
| `get_loki_label_cardinality`      | Loki        | Measure per-label cardinality and classify into bands               | `datasources:query`                     | `datasources:uid:loki-uid`                          |
| `audit_loki_label_strategy`       | Loki        | Audit a Loki label strategy (live or static) and return verdicts    | `datasources:query`                     | `datasources:uid:loki-uid`                          |
| `diagnose_loki_query_performance` | Loki        | Diagnose slow LogQL queries against the PS bottleneck checklist     | `datasources:query`                     | `datasources:uid:loki-uid`                          |
| `suggest_loki_alloy_label_config` | Loki        | Generate an Alloy `loki.process` snippet enforcing approved labels  | N/A                                     | N/A                                                 |
