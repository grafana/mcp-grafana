...
async def test_list_tools_result_includes_required_fields(grafana_env):
    ...
    try:
        ...
        # cacheScope and ttlMs come from the go-sdk's Cacheable struct.
        # resultType is set by the go-sdk only for certain result types
        # (CallToolResult, InitializeResult) — ListToolsResult does not
        # include it. Python SDK 2.x tolerates its absence on tools/list
        # when using protocol version 2025-11-25+.
        ...
        assert result["cacheScope"] in ("private", "public")
