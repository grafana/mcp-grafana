...
async def test_clickhouse_list_tables(
    ...
    prompt = (
        "Can you list all tables in the 'test' database of the ClickHouse datasource? "
        "Please use only the necessary tools to get this information."
    )
...
async def test_clickhouse_describe_table(
    ...
    prompt = (
        "Can you describe the schema of the 'logs' table in the 'test' database of the ClickHouse datasource? "
        "Show me the column names and types. Please use only the necessary tools to get this information."
    )
    final_content, tools_called, mcp_server = await run_llm_tool_loop(
        model, mcp_client, mcp_transport, prompt
    )
