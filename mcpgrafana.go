...
func ComposedStdioContextFunc(config GrafanaConfig) server.StdioContextFunc {
	return ComposeStdioContextFuncs(
		...
		ExtractGrafanaAPIClientFromEnv,
	)
}
...
func ComposedSSEContextFunc(config GrafanaConfig, cache ...*ClientCache) server.SSEContextFunc {
	grafanaExtractor, incidentExtractor, apiExtractor := clientExtractors(cache)
...
func clientExtractors(cache []*ClientCache) (httpContextFunc, httpContextFunc, httpContextFunc) {
	if len(cache) > 0 && cache[0] != nil {
		return extractGrafanaClientCached(cache[0]), extractIncidentClientCached(cache[0]), extractAPIClientCached(cache[0])
	}
	return ExtractGrafanaClientFromHeaders, ExtractIncidentClientFromHeaders, ExtractGrafanaAPIClientFromHeaders
}
